...
{
  if (max_out < _goboringcrypto_RSA_size(rsa)) {
    return 0;
  }
  int ret = _goboringcrypto_RSA_public_decrypt (in_len, in, out, rsa, padding);
  if (ret <= 0) {
    return 0;
  }
  *out_len = ret;
...
int _goboringcrypto_RSA_sign_raw(GO_RSA *rsa, size_t *out_len, uint8_t *out, size_t max_out,
  ...
  if (max_out < _goboringcrypto_RSA_size(rsa))
    return 0;
  int ret = _goboringcrypto_RSA_private_encrypt (in_len, in, out, rsa, padding);
  if (ret <= 0)
    return 0;
  *out_len = ret;
  return 1;
